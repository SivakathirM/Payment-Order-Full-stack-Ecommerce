...
const AllOrder = () => {
  ...
  return (
    <div className='h-[calc(90vh-130px)] overflow-y-scroll'>
      {
        !data[0]&&(
      ...
      <div className='p-4 w-full'>
        {
          data.map((item,index)=>{
            return(
              <div key={item.userId+index} className='mb-5'>
                        ...
                        item?.productDetails.map((product,index)=>{
                          return(
                            <div key={[product.productId+index]} className='flex gap-3 bg-slate-100'>
                              ...
                              />
                              <div>
                                <div className='font-medium text-lg text-ellipsis line-clamp-3'>{product.name}</div>
                          ...
                          <div className='text-lg font-medium'>Shipping Details : </div>
                            {
                              item.shipping_options.map((shipping,index)=>{
                                return (
                                  <div key={shipping.shipping_rate} className='ml-2'>Shipping Amount : {shipping.shipping_amount}</div>
                                )
                              })
                            }
                        </div>
