...
const Cart = () => {
  ...
  const totalQty=data.reduce((previoiusValue,currentValue)=>previoiusValue + currentValue.quantity,0)
        ...
        <div className='w-full max-w-3xl'>
          {
            loading ? (
              loadingCard.map((el,index)=>{
                ...
                </div> //remove key to loading problem solved
                )
              })
            ): (
              data.map((product,index)=>{
                return(
                  <div key={product?._id+"Add To Card Loading"} className='w-full bg-white h-auto my-2 border border-slate-300 rounded grid grid-cols-[128px,1fr]'>
                    <div className='w-32 h-32 bg-slate-200'>
                        <img src={product?.productId?.productImage[0] } className='w-full h-full object-scale-down mix-blend-multiply' />
                    </div>
                    <div className='px-4 py-2 relative'>
                      {/* delete product */}
                      <div className='absolute right-0 text-red-600 rounded-full p-2 hover:bg-red-600 hover:text-white cursor-pointer' onClick={()=> deleteCardProduct(product?._id)}>
                        <MdDelete />
                      </div>
                      ...
                      <div className='flex items-center gap-3 mt-2'>
                        <button className='p-1 border border-red-600 text-red-600 w-6 h-6 flex justify-center items-center rounded hover:bg-red-600 hover:text-white' onClick={()=> decreaseQty(product?._id,product?.quantity)}>-</button>
